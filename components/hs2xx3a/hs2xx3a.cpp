...
static const char *const TAG = "hs2xx3a";

void HS2XX3A::publishTarget(std::string idx, float dist, float snr) {
...
void HS2XX3A::publishNumber(std::string sensor, float resp) {
...
void HS2XX3A::publishSwitch(std::string sensor, int state) {
...
void HS2XX3A::getmmwConf(std::string mmwparam) {
  mmwparam = mmwparam + "\r";
  write_array(std::vector<unsigned char>(mmwparam.begin(), mmwparam.end()));
}

int HS2XX3A::readline(int readch, char *buffer, int len) {
  static int pos = 0;
  int rpos;
...
//           // zero null targets
//           for (int i = parse_number<int>(v[0]).value() + 1; i < 9; i++)
//             publishTarget(to_string(i), 0, 0);
//         }
      }
